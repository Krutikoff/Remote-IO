...
void PollingRegistersModule::_polling_gpio_registers()
{
    ...
    auto gpio_setup_registers = modbus.get_iterator<GpioModule::GpioMode>(
        Coil::GPIO_SET_BIT0);
    ...
    for (uint32_t i = 0U; i < _cached_gpio_mode.size(); ++i) {
        ...
        if (_cached_gpio_mode[i] != current_pin_mode) {
            ...
            _gpio_msg.action =
                (current_pin_mode == GpioModule::GpioMode::WRITE) ?
                    GpioModule::Action::SETUP_WRITE : GpioModule::Action::SETUP_READ;
    ...
    auto gpio_write_registers = modbus.get_iterator<uint8_t>(Coil::GPIO_WRITE_BIT0);
...
void PollingRegistersModule::_polling_latch_registers()
{
    Modbus& modbus = Modbus::instance();
    //Latch Clear
    auto latch_status = modbus.get_iterator<GpioModule::LatchStatus>(Coil::LATCH_CLEAR);

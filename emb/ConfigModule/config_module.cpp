...
void Module::mb_gpio_config(){
	...
	// Конфигурация GPIO для считывания адреса. С подтяжкой к питанию через регистр ODR
	gpio_set_mode(GPIOA, GPIO_MODE_INPUT, GPIO_CNF_INPUT_PULL_UPDOWN, GPIO4 | GPIO5 | GPIO6 | GPIO7);
	gpio_set(GPIOA, GPIO4 | GPIO5 | GPIO6 | GPIO7);
	// Конфигурация GPIO на вход порта С
	gpio_set_mode(GPIOC, GPIO_MODE_INPUT, GPIO_CNF_INPUT_FLOAT, GPIO0 | GPIO1 | GPIO2 | GPIO3 | GPIO4 | GPIO5 | GPIO6 | GPIO7);

}

void Module::mb_exti_config(){
	...
	exti_select_source(EXTI0 | EXTI1 | EXTI2 | EXTI3 | EXTI4| EXTI5 | EXTI6 | EXTI7, GPIOC);
	//exti_select_source(EXTI1, GPIOC);

	exti_set_trigger(EXTI0 | EXTI1 | EXTI2 | EXTI3 | EXTI4| EXTI5 | EXTI6 | EXTI7, EXTI_TRIGGER_BOTH);
	//exti_set_trigger(EXTI1, EXTI_TRIGGER_FALLING);

	exti_enable_request(EXTI0 | EXTI1 | EXTI2 | EXTI3 | EXTI4| EXTI5 | EXTI6 | EXTI7);
...
void Module::tim3_config() { // НЕ ВЫЗЫВАЕТСЯ
	...
	timer_set_prescaler(TIM3, (rcc_apb1_frequency * 2) / 100000); /* 72MHz to 50 microseconds period */
...
void tim3_isr(void){ // НЕ ВЫЗЫВАЕТСЯ
	TIM_SR(TIM3) &= ~TIM_SR_UIF; /* Clear interrrupt flag. */
}



void Module::Conf_PWM_TIM(uint32_t tim) {
...
void Module::usart_setup() { // НЕ ВЫЗЫВАЕТСЯ
